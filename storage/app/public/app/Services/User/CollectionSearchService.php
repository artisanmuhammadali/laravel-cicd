...
class CollectionSearchService {
    ...
    public function searchList($request,$type,$is_all = null)
    {
        ...
        $query1 = MtgUserCollection::where('user_id',$user->id)
                                    ->where('price','!=',0)
                                    ->where('quantity','!=',0)
        ...
        $cols = MtgUserCollection::where('user_id',$user->id)->with('card')
        ...
        ->when($request->code,function($q)use($request){
            $q->whereHas('card',function($que)use($request){
                $que->where('set_code',$request->code);
            });
        })
        ...
        ->when(!$request->ajax() || !$att ,function($q){
            $q->orderBy('created_at','desc')->orderBy('card_name','asc');
        });
