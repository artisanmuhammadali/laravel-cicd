    ...
    <table class="table table-hover">
        ...
        <tbody>
            ...
            <tr class="itemRow{{$item->id}}">
                <td class="py-0 px-25">
                    <input type="hidden" name="sw_card_id[{{$loop->iteration - 1}}]" value="{{$item->id}}">
                ...
                <td class="py-0 px-25">
                    <div title="{{$item->set->name.'-'.ucfirst($item->rarity)}}" class="text-center">
                        <img loading="lazy" alt="{{$item->set->name.'-'.ucfirst($item->rarity)}}" src="{{$item->set->icon ? $item->set->icon : getStarWarIcon()}}" style="filter: url({{"#".$item->rarity."_rarity"}});" width="25" alt="{{$item->set->name}}" >
                    </div>
                </td>
                <td class="py-0 px-25">
