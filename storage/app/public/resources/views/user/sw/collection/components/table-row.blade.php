<tr class="publish_check {{$item->publish ? 'published_item' : 'unpublished_item'}}">
    ...
    <td class=" align-items-center px-0">
        ...
        @if($item->card)
            <a href="{{route('sw.expansion.detail',[$item->card->set->slug, $item->card->url_type ,$item->card->slug])}}" target="_blank" class="fw-bold fs-12p-sm">{{$item->card->name}}</a>
    ...
    <td class="px-0">
        <div class="d-md-flex d-none">
            <button type="button" class="btn btn-icon btn-outline-primary waves-effect me-1 open_modal" data-url="{{route('user.collection.sw.edit',$item->id)}}" title="Edit">
                <i class="fa fa-pencil" aria-hidden="true"></i>
            </button>
            <button type="button" class="btn btn-icon btn-outline-danger waves-effect " onclick="deleteAlert('{{route('user.collection.sw.delete',$item->id)}}')" title="Delete">
        ...
        <div class="btn-group d-block d-md-none">
            ...
            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton2" style="">
                <a class="dropdown-item open_modal" href="#" data-url="{{route('user.collection.sw.edit',$item->id)}}" title="Edit">
                        Edit
                </a>
                <a class="dropdown-item" href="#" onclick="deleteAlert('{{route('user.collection.sw.delete',$item->id)}}')" title="Delete">
                        Delete
                </a>
            </div>
        </div>
