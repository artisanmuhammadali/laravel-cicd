<div class="row px-md-4 px-2">
    <div class="col-lg-3 col-12 p-0 d-flex justify-content-center">
        ...
        <div class="flip-card-3D-wrapper px-0">
            <div class="flip-card">
                @include('front.sw.components.singles.double-face',['class'=>'front','img'=>$item->front_image ?? null])
                @include('front.sw.components.singles.double-face',['class'=>'back','img'=>$item->back_image ?? null])
            </div>
        </div>
        @else
    ...
    <div class="col-lg-4 bg-gry d-lg-block d-none pt-5 my-3">
        <div class="row px-1">
            <div class="col-lg-8 pe-0">
                <h2 class="fs-5">{{$item->name ?? $item->name}}</h2>
            </div>
            @php($mana_cost=getCardManaCost($item->id))
            ...
            <div class="col-12 d-flex align-items-center mt-4">
                ...
                <p class="mb-0 ms-auto fs-6">#{{$item->hp}}</p>
            </div>
                    ...
                    <div class="row justify-content-center">
                        <ul class="hide ver_drop overflow-auto version_drop col-12 px-0" aria-labelledby="">
                            ...
                            <li class="drop-hover" style="position:relative">
                                {{-- <p>{{$version->name}}</p> --}}
                                <div class="btn d-flex expansion_detailed_route" data-url="{{route('sw.expansion.detail',[$version->set->slug, $version->url_type ,$version->slug])}}">
                                    ...
                                    <img loading="lazy" class=" img-fluid"
                                            ...
                                            alt="">
                                    <p class="fs-6 bolder my-auto ms-1 text-truncate pe-1" title="{{$version->name}}">{{$version->name}}  {{$version->parent_id ?? ""}}
                            ...
                            <div class="col-12 d-flex justify-content-center">
                                <a href="{{route('sw.specific.search','items')}}?keyword={{$item->name}}&item=versions" class="btn btn-site-primary w-100 rounded-0">View All {{$item->versions->count()}} Items</a>
                            </div>
                        </ul>
                    </div>
                </div>
