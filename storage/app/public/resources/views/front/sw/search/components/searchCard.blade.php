        ...
        @php($active = $tabb == 'item' ? 'active' : '')
            <div class="tab-pane {{$active}}" id="item-fill" role="tabpanel" aria-labelledby="item-tab-fill">
                <div class="inner_search_box">
                    <table class="table table-striped">
                        <tbody>
                            ...
                            <tr class="expansion_detailed_route" data-url="{{route('sw.expansion.detail',[$item->set->slug, $item->url_type ,$item->slug])}}">
                ...
                <div class="text-center">
                    <a href="{{route('sw.specific.search','items')}}?keyword={{$request->keyword}}" class="btn btn-site-primary mt-1">View All {{$item_count}} Items</a>
                </div>
            </div>
            @php($active = $tabb == 'expansion' ? 'active' : '')
                ...
                <div class="inner_search_box">
                    <table class="table table-striped">
                        <tbody>
                            ...
                            <tr class="expansion_detailed_route" data-url="{{ route('sw.expansion.set', $item->slug) }}" >
                                <th scope="row"><img src="{{getStarWarIcon()}}" width="50px"></th>
                                <td>{{$item->name}}</td>
                                <td>{{$item->code}}</td>
                            </tr>
