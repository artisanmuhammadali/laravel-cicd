<div class="d-flex">
    @if($item->is_active)
        <a href="{{ route('admin.mtg.sets.edit', $item->id) }}"
            class="btn btn-primary w-50p dropdown-item me-1" title="Edit">
            <i class='fa fa-edit'></i>
        </a>
        <a href="{{ route('admin.mtg.sets.seo', $item->id) }}"
        ...
        <a onclick="confirmationAlert('{{ route('admin.mtg.sets.inactive', $item->id) }}')"
    ...
    @else
        <a data-target="#attributeModal" title="Update Release Date" class="btn btn-warning open_modal me-25"
            data-url="{{ route('admin.mtg.sets.modal', $item->id) }}" data-type="date">
            <i class='fa fa-edit'></i>
        </a>
        <a  title="Update Set" class="btn btn-secondary  me-25"
            href="{{ route('admin.mtg.sets.api.update', $item->code) }}">
            <i class='fa fa-edit'></i>
        </a>
        @if($item->released_at <= now()->format('Y-m-d') )
            <a data-target="#attributeModal" title="Active Set" class="btn btn-primary open_modal"
                data-url="{{ route('admin.mtg.sets.modal', $item->id) }}"><i
